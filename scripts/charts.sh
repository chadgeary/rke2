...
# helm
if [ -f "$PLATFORM_BIN_PATH/helm" ]; then
    echo "INFO: helm exists, skipping"
else
    aws --region "$REGION" s3 cp s3://"$PREFIX"-"$SUFFIX"-private/data/downloads/rke2/helm-"$ARCH".tar.gz /opt/helm-"$ARCH".tar.gz
    if [ "$ARCH" == "arm64" ]; then
        tar -zx -f /opt/helm-"$ARCH".tar.gz --strip-components=1 --directory "$PLATFORM_BIN_PATH" "linux-arm64/helm"
    else
        tar -zx -f /opt/helm-"$ARCH".tar.gz --strip-components=1 --directory "$PLATFORM_BIN_PATH" "linux-amd64/helm"
    fi
    chmod +x "$PLATFORM_BIN_PATH"/helm
fi

# charts from tf -> getfiles lambda -> s3
mkdir -p "$CHARTS_PATH"
...
# cilium-secret
until helm --kube-apiserver https://localhost:6443 --kubeconfig /etc/rancher/rke2/rke2.yaml upgrade --install \
...
helm --kube-apiserver https://localhost:6443 --kubeconfig /etc/rancher/rke2/rke2.yaml upgrade --install \
