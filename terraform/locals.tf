locals {
  ...
  azs    = slice(data.aws_availability_zones.rke2.names, 0, var.azs)
  ...
  subnet-vpce = merge([for subnet in aws_subnet.rke2-private : { for vpce in aws_vpc_endpoint.rke2-vpces : "${subnet.availability_zone}-${strrev(element(split(".", strrev(vpce.service_name)), 0))}" => { "subnet" = subnet.id, "vpce" = vpce.id } }]...)
  ...
  ecr_pull_through_caches = data.aws_partition.rke2.partition == "aws" ? {
    quay = "quay.io"
    ecr  = "public.ecr.aws"
  } : {}
